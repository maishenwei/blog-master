<template>
    <div class="editor-box" style="line-height: 0;">
        ...
        <div>
            <input style="width: 99%;height: 30px;margin-top: 10px;" v-model="title" placeholder="请输入标题" />
        </div>

        <div >
            <textarea style = "width: 99%;margin-top: 10px;resize: none;overflow: auto;"
            v-model="introduce"
            rows="4"
            maxlength="500"
            placeholder="请输入摘要"
        ...
        <div>
            <input style="width: 30%;height: 30px;margin-top: 10px;" v-model="tagname" placeholder="请输入标签" />
        </div>
        <!-- 图片上传组件辅助-->
        ...
        <div style="float:left;width:97%;margin-left:1.5%;margin-top: 10px;margin-bottom: 10px;">
            <el-button style="margin-left: 20px;float:right;" type="primary" @click="postBlog()">发表</el-button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        postBlog() {
            ...
            axios.post("/post/post",postdata).then(function(response){
